...
public class GlobalExceptionHandler {
     ...
     */
    @ExceptionHandler(ApplicationExceptions.class)
    public ResponseEntity<ApiResponse<Void>> handleApplicationException(ApplicationExceptions ex) {
        // 예외 세부 정보 로깅 (로그 레벨을 ERROR로 사용)
        log.error("[애플리케이션 예외] 코드: {}, 메시지: {}", ex.getErrorCode(), ex.getMessage(), ex);

        // 에러 코드에 적합한 HTTP 상태 코드 결정 (매핑 테이블 참조)
        HttpStatus status = ERROR_CODE_MAP.getOrDefault(ex.getErrorCode(), HttpStatus.INTERNAL_SERVER_ERROR);
    ...
    public ResponseEntity<ApiResponse<Map<String, String>>> handleValidationException(
        ...
        return ResponseEntity
                ...
                .body(ApiResponse.error(HttpStatus.BAD_REQUEST.value(), "입력값 검증에 실패했습니다"));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleMethodArgumentTypeMismatch(MethodArgumentTypeMismatchException ex) {
        log.error("[파라미터 타입 불일치] {}", ex.getMessage());

        return ResponseEntity
                .status(HttpStatus.BAD_REQUEST)
                .body(ApiResponse.error(HttpStatus.BAD_REQUEST.value(),
                        String.format("파라미터 '%s'의 값 '%s'이(가) 예상 타입과 일치하지 않습니다",
    ...
    public ResponseEntity<ApiResponse<Void>> handleGenericException(Exception ex) {
        // 스택 트레이스와 함께 상세 로깅
        log.error("[처리되지 않은 예외] 타입: {}, 메시지: {}", ex.getClass().getName(), ex.getMessage(), ex);
